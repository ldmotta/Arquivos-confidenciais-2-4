	...
	If (!_inbuilding) then {
		...
		{
			{
				...
				If ((_unit distance _x) <= 2) exitwith {_watch = _x;};
		...
		If (!_ouverture) then
		{
			...
			if (count _doorposition == 0) then 
			{
				...
				If (count _exitpos > 0) then {_doorposition = _doorposition + [_bld modelToWorld _exitpos]};
			};
		...
		sleep 0.1;
	{
		...
		if (!(typeof _bld in MON_Bld_ruins)) then 
		{
			If (count _windowsposition > 0) then 
			{
				{
					_windowpos1 = _x;
					If (_bldpos1 distance _windowpos1 <= 4) then {_windowspos = _windowspos + [_bldpos1]; _loop = false;};
			...
			If (count _doorsposition > 0 && _loop) then 
			{
				{
					_doorpos1 = _x;
					If (_bldpos1 distance _doorpos1 <= 5) then {_doorspos = _doorspos + [_bldpos1]; _loop = false;};			
...
MON_moveBuildings2 = {
	...
	//if (KRON_UPS_Debug>0) then {diag_log format["MON_moveBuildings _units=%1 _blds=%2",count _units, count _blds];};	
	{
		...
		if (count _bldpos >= 10) then {_cntobjs1 =   round(random 4)  + 4;};
		if (count _blds < 2 && count _bldpos >= count _units) then {_cntobjs1 = count _units};		
			...
			if (count _movein > 0) then 
			{
			...
			
				{
					...
					_altura = [_bldpos] call Aze_Checkfreebldpos;
					If (count _altura > 0) then {[_x,_bld,_altura,_wait] spawn MON_movetoBuilding2;} else {_units = _units + [_x];};
					_bldpos set [0,1];
					_bldpos = _bldpos - [0];
					sleep 0.3;
					_bldpos = _bldpos - [1];	
...
MON_movetoBuilding2 = {
	...
	if (_retry) then {	
		...
		sleep 0.5;
		_allpos = [_bld,_bldpos,"RANDOMA"] call Aze_SortOutBldpos2;
		sleep 0.5;
		...
		[_npc,_bld,_altura] spawn MON_movetoBuilding2;
...
Aze_Checkfreebldpos = {
	...
	{
		_bldpos1 = _x;
		_unitnear =  _bldpos1 nearentities ["CAManBase",0.5];
		If (count _unitnear == 0) exitwith {_altura = _bldpos1};
	} count _bldpos > 0;
...
Aze_findnearestenemy = {
	...
	If (count _enemies > 0) then
	{
		...
		If (_shareinfo=="SHARE") then 
		{
			{
				_alliednpc = _x;
				If (side _npc == side _alliednpc && _npc distance _alliednpc <= KRON_UPS_sharedist) then {{_enemy = _x; _alliednpc reveal [_enemy,1.5];} foreach _enemies;};	
			} count KRON_NPCs > 0;
		};	
	};
