...
//if is vehicle will not be in units so must set manually
	...
	if (isnil {_npc getVariable ("UPSMON_grpid")}) then {		
		{_x setVariable ["UPSMON_grpid", _grpid, false];} foreach units _npc;
		sleep 0.05;
...
if (UPSMON_Debug>0) then {diag_log format["UPSMON - npc [%1] - typename [%2] - typeof [%3] - units [%4]",_npc, typename _npc, typeof _npc, units (group _npc)];};	
	...
	{		
		...
		{
			_npc = ([_npc,(group _npc)] call UPSMON_getleader);
		}
		else 
		{
	...
	else 
	{
		if (count _obj>0) then {
			_npc = [_obj,group _obj] call UPSMON_getleader;			
		};
	};
...
//Fills member soldier types
	{	
		...
		if (vehicle _x == _x || (vehicle _x != _x && (driver vehicle _x != _x || gunner vehicle _x != _x || commander vehicle _x != _x))) then {_membertypes = _membertypes + [[typeof _x,_equipment]];};
	} foreach _members;
...
//Fills member vehicle types
	{
		_dirveh = getdir _x;
		_crew = [];
		If (!(isNull (driver _x))) then {_crew = _crew + [typeof (driver _x)];};
		If (!(isNull (gunner _x))) then {_crew = _crew + [typeof (gunner _x)];};
		If (!(isNull (commander _x))) then {_crew = _crew + [typeof (commander _x)];};
		_vehicletypes = _vehicletypes + [typeof _x,_dirveh,_crew];
	} foreach _vehicles;
...
If (_reinforcement == "REINFORCEMENT") then {
	switch (_side) do {
		...
		UPSMON_REINFORCEMENT_WEST_UNITS = UPSMON_REINFORCEMENT_WEST_UNITS + [_grp];
		...
		UPSMON_REINFORCEMENT_EAST_UNITS = UPSMON_REINFORCEMENT_EAST_UNITS + [_grp];
		...
		UPSMON_REINFORCEMENT_GUER_UNITS = UPSMON_REINFORCEMENT_GUER_UNITS + [_grp];
...
if ("LIMITED" in _UCthis) then {_speedmode = "LIMITED"};
if ("NORMAL" in _UCthis) then {_speedmode = "NORMAL"}; 
if ("FULL" in _UCthis || _noslow == "NOSLOW") then {_speedmode = "FULL"};
...
// make start position random 
	if (_initpos!="ORIGINAL") then {
		...
		If ((_initpos=="RANDOMUP") || (_initpos=="RANDOMDN") || (_initpos=="RANDOMA")) then
		{
			...
			if (count _OCercanos > 1) then {_OCercanos = _OCercanos call UPSMON_arrayShufflePlus;};
			
			{
				...
				_bldpositions set [count _bldpositions, [_x,_allpos]];
				If (_nbbldpos >= 2*(count units _npc)) exitwith {_bldpositions};
			}foreach _OCercanos;
		};
		...
		if (_nbbldpos <= 0) then 
		{
			...
			{ // vehicles
				_targetpos = [];
				If (!_isboat) then {_targetpos = _currPos findEmptyPosition [0,50];};
				sleep .05;						
				if (count _targetpos <= 0) then {_targetpos = _currpos};
				_x setPos _targetpos;
			} foreach _vehicles;
		...
		// put the unit on top of a building
			...
			If (_nowpType == 3) then 
			{
				_unitsIn = [_grpid,_npc,_area,true] call UPSMON_GetIn_NearestStatic;
				if ( count _unitsIn > 0 ) then {_units = units _npc - [_unitsIn];};
				_grpmission = "FORTIFY";
			};
			[_units,_bldpositions,_npc,0] spawn UPSMON_SpawninBuildings;
			_currPos = getPosATL _npc;
...
If (_nowp && _grpmission != "FORTIFY") then {_grpmission = "NOWP";};
...
//If a soldier has a useful building takes about ======================================================================================================================		
	...
	{			
		...
		_units = _members;
		if ( count _unitsIn > 0 ) then {sleep 2; _units = units _npc - [_unitsIn];};
		_blds = [_npc,70,false,9999,true] call UPSMON_moveNearestBuildings;
		_grpmission = "FORTIFY";
		If (count _blds == 0) then {_lookpos = [getposATL _npc,getdir _npc, 20] call UPSMON_GetPos2D;[getposATL _npc,_lookpos,50,false,_units] call UPSMON_fnc_find_cover;};
		_grp enableAttack false;
		_nowp = true;
		_nowpType = 3;
	};
	...
	If (_ambush) then 
	{
		...
		_positiontoambush = [_grp,_Ucthis] call UPSMON_getAmbushpos;
		_grpmission = "AMBUSH";
	...
	_static = [_grp] call UPSMON_AddArtillery;
	if (_static && !_nowp) then {_nowp = true;_nowpType = 2;};
	...
	// variables
	_grp setvariable ["UPSMON_Grpstatus",[_grpmission,"GREEN",_grpmission,[0,0,UPSMON_minreact],ObjNull,[0,0]]];
	_grp setvariable ["UPSMON_Lastinfos",[[0,0,0],"MOVE",[0,0,0],0,objNull,0,units _npc,0,[0,0],[0,0]]];
	_grp setvariable ["UPSMON_RadioRange",_RadioRange];
	_grp setvariable ["UPSMON_Origin",[_orgMode,_speedmode,_orgpos,_orgformation,_members]];
	_grp setvariable ["UPSMON_NOWP",[_nowp,_nowpType]];
	_grp setvariable ["UPSMON_Fortify",[_fortifyorig,_fortify]];
	_grp setvariable ["UPSMON_REINFORCEMENTSENT",[false,0]];
	_grp setvariable ["UPSMON_REINFORCEMENT",_reinforcement];
	_grp setvariable ["UPSMON_TimeArtillery",30];
	_grp setvariable ["UPSMON_RFID",_rfid];
	_grp setvariable ["UPSMON_RESPAWNARRAY",[_membertypes,_vehicletypes]];
	_grp setvariable ["UPSMON_TIMEONTARGET",[_time,0,_time]];
	_grp setvariable ["UPSMON_makenewtarget",true];
	_grp setvariable ["UPSMON_Removegroup",false];
	_grp setvariable ["UPSMON_DskHeli",false];
	_grp setvariable ["UPSMON_REACT",[0,0,UPSMON_minreact]];
	_grp setvariable ["UPSMON_PosToRenf",[0,0]];
	_grp setvariable ["UPSMON_searchingpos",false];
	_grp setvariable ["UPSMON_targetPos",[0,0]];
